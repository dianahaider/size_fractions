...
def rarefy_and_calc_metrics(counts: pd.Series, depth: int):
    total_reads = int(counts.sum())
    if total_reads == 0:
        return np.nan, np.nan, np.nan

    population = np.repeat(counts.index.values, counts.values.astype(int))
    # switch to replacement if not enough reads
    replace = total_reads < depth

    rarefied = np.random.choice(population, size=depth, replace=replace)
    rare_counts = pd.Series(rarefied).value_counts()

    richness = rare_counts.count()
    freqs = rare_counts / depth
    shannon = -(freqs * np.log(freqs)).sum()
    evenness = shannon / np.log(richness) if richness > 0 else np.nan

    return richness, shannon, evenness

def repeat_rarefy(table: pd.DataFrame, depth: int, n_iter: int = 100):
    runs = []
    for i in range(1, n_iter+1):
        df_metrics = table.apply(lambda row: rarefy_and_calc_metrics(row, depth), axis=1)
        df_metrics = pd.DataFrame(df_metrics.tolist(),
                                  index=table.index,
                                  columns=[f"richness_{i}", f"shannon_{i}", f"evenness_{i}"])
        runs.append(df_metrics)
    return pd.concat(runs, axis=1)
