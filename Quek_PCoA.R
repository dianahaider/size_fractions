...
directory = "C:\\Users\\rbeik\\OneDrive - Dalhousie University\\Projects\\Kenya-Vaginal\\Quek-neonatal\\Aug2023\\output-NoP\\diversity_sdepth_2000\\exported_distance_matrix\\"
infile = "WUF-distance-matrix.tsv"
outfile = "WUF-PCoA-plt.png"
...
type_colors = c("#4477AA","#EE7733","#BBBBBB")

# Add points based on the ending character of labels
colors <- ifelse(grepl("V$", rownames(pcoa_result$vectors)), type_colors[1],
                 ifelse(grepl("R$", rownames(pcoa_result$vectors)), type_colors[2],
                        ifelse(grepl("[SF]$", rownames(pcoa_result$vectors)), type_colors[3], "black"))) # "black" as a default, just in case

points(pcoa_result$vectors[,1], pcoa_result$vectors[,2], pch = 21, bg = colors, cex = 1.6)

# Add a legend with circle markers
legend("topright", 
       legend = c("Vaginal", "Rectal", "Fecal"), 
       col = type_colors, 
...
group_labels <- ifelse(grepl("V$", dist_matrix_rownames), "Vaginal",
                       ifelse(grepl("R$", dist_matrix_rownames), "Rectal",
                              ifelse(grepl("[SF]$", dist_matrix_rownames), "Fecal", NA)))
